...
export class ParityRPC {
  ...
  private async transactionFromParityTrace(tx: ParityTraceResponse, chain: string, network: string) {
    ...
    return {
      ...
      from: tx.action.from!,
      to: tx.action.to!,
      category: decodedData && decodedData.params ? 'transfer' : tx.type,
      ERC20: decodedData && decodedData.params ? true : false,
      tokenTransfer:
        decodedData && decodedData.params ? decodedData.params.filter(e => e.name === '_value')[0].value : 0
    };
  }
}
