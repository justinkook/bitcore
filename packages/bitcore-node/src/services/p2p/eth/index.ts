...
import { IBlock, EthBlockStorage } from '../../../models/block/eth';
...
export class EthP2pService {
  ...
  async processBlock(block: Ethereum.Block): Promise<any> {
    ...
    const transactions = await new ParityRPC(this.web3).getTransactionsFromBlock(
      convertedBlock.height,
      this.chain,
      this.network
    );
    for await (const tx of transactions) {
      const foundIndex = convertedTransactions.findIndex(t => t.txid === tx.txid);
      if (foundIndex > -1) {
        convertedTransactions[foundIndex] = this.convertTransaction(tx, convertedBlock);
      } else {
        convertedTransactions = [...convertedTransactions, this.convertTransaction(tx, convertedBlock)];
      }
    }
    await EthBlockStorage.addEthBlock({
      chain: this.chain,
      network: this.network,
  ...
  async processTransaction(tx: Ethereum.Transaction): Promise<any> {
    ...
    const now = new Date();
    TransactionStorage.batchEthImport({
      chain: this.chain,
      network: this.network,
      txs: [convertedTx],
      height: -1,
